...
class LogStorage implements \Tc\Interfaces\Wizard\LogStorage
{
	...
	public function getLogs(\User $user = null): array {
		return array_map(fn ($log) => new Log($log), $this->getUserLogs($user));
	}
	...
	public function getLastLog(\User $user = null): ?LogInterface
	{
		...
		if (null === $last = Arr::last($this->getUserLogs($user))) {
			return null;
		}
	...
	{
		if ($specific === null) {
			$others = array_filter($this->saved, fn ($log) => (int)$log['user_id'] !== (int)$user->id);
			return $this->write($others);
		}
	...
	protected function getUserLogs(\User $user): array
	{
		return array_filter($this->saved, fn ($log) => (int)$log['user_id'] === (int)$user->id);
	}
	...
	protected function write(array $logs): static
	{
		...
		if (!is_writable($dir)) {
			throw new \RuntimeException(sprintf('Directory is not writeable [%s]', $dir));
		}
	...
	{
		$dir = storage_path('wizard');
		if (!is_dir($dir)) {
			\Util::checkDir($dir);
		}
		return $dir.'/email_accounts.txt';
	}
