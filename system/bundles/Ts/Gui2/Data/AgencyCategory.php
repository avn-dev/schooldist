...
class AgencyCategory extends \Ext_Thebing_Gui2_Data
{
	...
	static public function getDialog(\Ext_Thebing_Gui2 $oGui)
    {
		...
		$oDialog = $oGui->createDialog(
			$oGui->t('Agenturkategorie "{name}" editieren'),
			$oGui->t('Neue Agenturkategorie anlegen')
		);
		...
		$oDialog->setElement($oDialog->createRow(
			$oGui->t('Bezeichnung'), 'input', array('db_alias' => '', 'db_column'=>'name','required' => 1))
		);
