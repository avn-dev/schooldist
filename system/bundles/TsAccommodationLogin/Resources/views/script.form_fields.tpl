			...
			{foreach item=aField from=$aFields}
				...
				{else}
					...
					<div class="form-group">
					   <label for="street" class="col-sm-2 control-label">{$aField.label}</label>
					   ...
					   <div class="col-sm-10">
						...
						{if $sTask == 'add'}
							{if $aField.type == 'select'}
								...
								<select name="{$aField.field}" class="form-control" onchange="{$aField.onchange} return false;">
									{html_options options=$aField.data_array selected=$aField.value}
								</select>
						
							{elseif $aField.type == 'date'}
								<input type="text" class="form-control inputDatePicker" id="{$aField.field}" name="{$aField.field}" value="{$aField.value|date_format:$aField.date_format}" />
							{elseif $aField.type == 'time'}
								<input type="text" class="form-control" id="{$aField.field}" name="{$aField.field}" value="{$aField.value|date_format:"%X"}" />
							{elseif $aField.type == 'image' || $aField.type == 'file'}
								<input type="file" class="form-control" id="{$aField.field}" name="{$aField.field}" value="{$aField.value|escape}" />
							{elseif $aField.type == 'textarea'}
								<textarea class="form-control" id="{$aField.field}" name="{$aField.field}" style="{$aField.style}">{$aField.value|escape}</textarea>
							{elseif $aField.type == 'html'}								
								<textarea class="form-control summernote" id="{$aField.field}" name="{$aField.field}" style="{$aField.style}">{$aField.value|escape}</textarea>
							{else}
								<input class="form-control" type="text" id="{$aField.field}" name="{$aField.field}" value="{$aField.value|escape}" />
							{/if}
						{else}
							{if $aField.type == 'select'}
