...
class TransferPayments implements Application
{
	public static function getTitle(LanguageAbstract $l10n, string $application): string
	{
		return $l10n->translate('Buchhaltung » Transferanbieter');
	}
	...
	public function getRecipients(LanguageAbstract $l10n, Collection $models, string $channel): AddressContactsCollection
	{
		...
		foreach ($models as $model) {
			$provider = $model->getItem();

			if ($provider instanceof \Ext_Thebing_Pickup_Company) {
				$collection->add(
					(new \Communication\Services\AddressBook\AddressBookContact('transfer.'.$provider->id, $provider))
						->groups($l10n->translate('Transferanbieter'))
						->recipients('transfer_provider')
						->source($model)
				);
			} else {
				$collection->add(
					(new \Communication\Services\AddressBook\AddressBookContact('accommodation.'.$provider->id, $provider))
	...
	{
		return $models->map(fn (\Ext_TS_Accounting_Provider_Grouping_Transfer $model) => $model->getItem());
	}

	public function getPlaceholderObject(\Ext_TS_Accounting_Provider_Grouping_Transfer $grouping, \Ext_TC_Communication_Template|null $template, Collection $to, string $language, bool $finalOutput)
	{
		if ($template && (bool)$template->legacy) {
			$transfer = $grouping->getItem();
			$placeholder = new \Ext_TS_Accounting_Provider_Grouping_Transfer_Placeholder($transfer, $grouping);
