...
class Agency extends Company {
	...
	public function switchAjaxRequest($_VARS)
	{
		...
		if($_VARS['action'] == 'extend_export'){
			...
			if($iTemplateId > 0) {
				...
				$overViewDate = $dateFormat->convert($_VARS['save']['overview_date']);
				...
				foreach((array)$aSelectedIds as $iAgencyId) {
					...
					$oReplace->setAdditionalData('overview_contacts', $_VARS['save']['company_contact_id']);
					$oReplace->setAdditionalData('overview_date', $overViewDate);
					$oReplace->setAdditionalData('overview_time', $_VARS['save']['overview_time']);

					$oVersion = new \Ext_Thebing_Inquiry_Document_Version();
					$oVersion->template_id = $iTemplateId;
	...
	public function getAgencyOverviewDialog(array $aSelectedIds) {
		...
		$oDialog = $this->_oGui->createDialog($this->_oGui->t('Agentur PDF'));
		$oDialog->width = 900;
		$oDialog->height = 650;
		$oDialog->sDialogIDTag = 'AGENCYOVERVIEW_';

		$oDialog->setElement($oDialog->createRow($this->_oGui->t('PDF-Vorlage'), 'select', [
