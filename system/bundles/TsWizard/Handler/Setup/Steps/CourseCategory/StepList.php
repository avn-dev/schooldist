...
class StepList extends Wizard\Structure\Step
{
	...
	public function getTable(Wizard $wizard, Request $request): Wizard\Structure\Table
	{
		...
		$existing = BlockCategories::entityQuery($school)->get();

		$table = (new Wizard\Structure\Table($existing))
			->column($wizard->translate('Kategorie'), function ($entity) {
				return $entity->getName();
			})
			->new($wizard->translate('Neue Kurskategorie'), $wizard->routeStep($this, 'step.save', ['action' => 'new', 'school_id' => $school->id]))
			...
			->delete($wizard->translate('Löschen'), $wizard->translate('Möchten Sie die Kurskategorie wirklich löschen?'),
	...
	public function action(Wizard $wizard, string $action, Request $request, $next): Response
	{
		...
		switch ($action) {
			...
			case 'delete':
				$entity = BlockCategories::entityQuery($school)->findOrFail($request->get('category_id', 0));
	...
	protected function save(Wizard $wizard, Request $request): ?MessageBag
	{
		...
		$first = BlockCategories::entityQuery($school)->first();

		if ($first === null) {
			return new MessageBag([$wizard->translate('Bitte fügen Sie mindestens eine Kurskategorie hinzu.')]);
		}
