...
class StepList extends Wizard\Structure\Step
{
	...
	public function getTable(Wizard $wizard, Request $request): Wizard\Structure\Table
	{
		...
		$existing = BlockPriceWeeks::entityQuery($school)->get();

		$table = (new Wizard\Structure\Table($existing))
			->column($wizard->translate('Bezeichnung'), function ($entity) {
				return $entity->title;
			})
			->new($wizard->translate('Neue Preiswoche'), $wizard->routeStep($this, 'step.save', ['action' => 'new', 'school_id' => $school->id]))
			->edit($wizard->translate('Editieren'), function ($entity) use ($wizard, $school) {
				return $wizard->routeStep($this, 'step.save', ['action' => 'edit', 'school_id' => $school->id, 'week_id' => $entity->id]);
			})
			->delete($wizard->translate('Löschen'), $wizard->translate('Möchten Sie die Preiswoche wirklich löschen?'),
				function ($entity) use ($wizard, $school) {
					return $wizard->routeStep($this, 'step.save', ['action' => 'delete', 'school_id' => $school->id, 'week_id' => $entity->id]);
				}
			)
		;
	...
	public function action(Wizard $wizard, string $action, Request $request, $next): Response
	{
		...
		switch ($action) {
			case 'new':
				...
				return $wizard->redirect($form, ['school_id' => $school->id, 'week_id' => 0]);
			case 'edit':
				$form = $this->parent->get('form');
				return $wizard->redirect($form, ['school_id' => $school->id, 'week_id' => $request->get('week_id')]);
			case 'delete':
				$entity = BlockPriceWeeks::entityQuery($school)->findOrFail($request->get('week_id', 0));
	...
	protected function save(Wizard $wizard, Request $request): ?MessageBag
	{
		...
		$first = BlockPriceWeeks::entityQuery($school)->first();

		if ($first === null) {
			return new MessageBag([$wizard->translate('Bitte fügen Sie mindestens eine Preiswoche hinzu.')]);
		}
