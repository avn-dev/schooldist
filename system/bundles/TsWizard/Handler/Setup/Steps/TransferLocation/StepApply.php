...
use TsWizard\Handler\Setup\Steps\CourseCategory\BlockCategories;
...
class StepApply extends Step
{
	...
	public function getTable(Wizard $wizard, Request $request): Wizard\Structure\Table
	{
		...
		$others = BlockTransferLocations::othersQuery($school)->get();

		$table = (new Wizard\Structure\Table($others))
			->column($wizard->translate('Reiseort'), function ($entity) {
				return $entity->getName();
			})
			->action($wizard->translate('Übernehmen'), 'fa fa-plus-circle', 'btn-info', function ($entity) use ($wizard, $school) {
				return $wizard->routeStep($this, 'step.save', ['action' => 'apply', 'school_id' => $school->id, 'location_id' => $entity->id]);
			})
		;
	...
	public function action(Wizard $wizard, string $action, Request $request, $next): Response
	{
		...
		switch ($action) {
			case 'apply':
				$entity = \Ext_TS_Transfer_Location::query()->findOrFail($request->get('location_id', 0));
