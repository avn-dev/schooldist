...
if($_VARS['task'] == "delete") {
	...
	db_query($db_data['module'],"DELETE FROM community_buddylist WHERE tblUser = '".$user_data['idTable']."' AND idUser = '".$user_data['id']."' AND id = '".$_VARS['idBuddy']."' ");
...
$res = db_query($db_data['module'],"SELECT id, idBuddy, tblBuddy FROM community_buddylist WHERE tblUser = '".$user_data['idTable']."' AND idUser = '".$user_data['id']."'");

while($my = get_data($res)) {
	$buffer = $buffer_entry;

	$my_buddy = get_data(db_query($db_data['module'],"SELECT c.id,c.nickname,UNIX_TIMESTAMP(w.changed) as lastaction FROM customer_db_".$my['tblBuddy']." AS c LEFT OUTER JOIN community_whoisonline AS w ON c.id = w.idUser WHERE c.id = '".$my['idBuddy']."'"));
	...
	$buffer = str_replace("<#tblUser#>",$my['tblBuddy'],$buffer);
