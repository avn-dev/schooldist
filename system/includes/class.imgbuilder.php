...
// TODO : ImageBuilder auf Speicherkapazität überprüfen!

/**
 * class.imgbuilder.php
 *
 * contains function for generating images with text and images 
 *
...
class imgBuilder {
	 ...
	 * imgBuilder
	 * 
	 * constructor of the imgbuilder class
	 * 
	 * @author	Mark Koopmann - m.koopmann@plan-i.de
	 */
	function __construct() {

		$this->strFontPath = \Util::getDocumentRoot()."storage/";
		$this->strImagePath = \Util::getDocumentRoot()."storage/public/";
		$this->strTargetPath = \Util::getDocumentRoot()."storage/public/tmp/";
		$this->strTargetUrl = "/storage/public/tmp/";
		$this->strImPath = '/usr/bin/';
		$this->intScaleFactor = 4;
		$this->intTempSize = 155;

		Util::checkDir($this->strTargetPath);

		// Die Bearbeitung von großen Bildern erfordert viel RAM
	...
	/**
	 * @param array $arrInfo
	 * @param boolean $bolReturn
	 * @param boolean $bolSave
	 * @param array $aSet
	 * @return string
	 */
	static function doImgBuilder($arrInfo, $bolReturn=0, $bolSave=1, $aSet=null) {
		$objImageBuilder = new imgBuilder();
		$mixReturn = $objImageBuilder->buildImage($arrInfo, $bolReturn, $bolSave, $aSet);
	...
	public function getImageObject($strFile) {
		...
		if(function_exists('finfo_open')) {
			...
			$sMimeType = finfo_file($rFinfo, $strFile);
		}
	...
	protected function getSet($iSetId) {
		
		$sCacheKey = __METHOD__.'_'.$iSetId;
		
		if(!isset(self::$aCache[$sCacheKey])) {
			self::$aCache[$sCacheKey] = DB::getQueryRow("SELECT * FROM system_imgbuilder WHERE id = ".(int)$iSetId." ");
		}
	 ...
	 */
	function buildImage($arrInfo, $bolReturn=0, $bolSave=1, $aSet=null) {
		global $system_data;

		$this->arrInfo['set'] = $arrInfo[1];
		
		$sPathReplaceCheck = str_replace(Util::getDocumentRoot(false), '', $this->strImagePath);
		
		// Get set info from set definition array
		if($aSet === null) {
			$aSet = $this->getSet((int)$this->arrInfo['set']);
			$aSet['data'] = Util::decodeSerializeOrJson($aSet['data']);
		}

		$aSet['data'] = (array)$aSet['data'];

		$sMD5Text = $aSet['date'];

		// loops every element
		foreach($aSet['data'] as $dkey => &$dval) {

			// if user input
			if ($dval['user'] == 1) {

				$strValue = stripslashes(html_entity_decode($arrInfo[$dval['index']+1]));
				...
				if(
					...
					// /storage/public entfernen, damit auch ein absoluter Pfad funktioniert.
					$strValue = str_replace($sPathReplaceCheck, '', $strValue);
					
					// Abwärtskompatibilität: Altes /media Verzeichnis entfernen
					if(strpos($strValue, "/media") === 0) {	
						$strValue = str_replace("/media/", "", $strValue);
					}

					if(
						is_file($this->strImagePath.$strValue) &&
						strpos(realpath($this->strImagePath.$strValue), realpath($this->strImagePath)) === 0
					) {
						$dval['file'] = $strValue;
		...
		if(!file_exists($strTargetPath)) {
			...
			foreach($aSet['data'] as $dkey => $dval) {
				...
				if ($dval['type'] == "fonts") {
					...
					if($dval['grayscale'] == 1) {
						imagefilter($rImgCopy, IMG_FILTER_GRAYSCALE);
					}

					// if not valid image resource or GdImage-Object (PHP8), continue with next element
					if (!is_resource($rImgCopy) && !is_object($rImgCopy)) {
						continue;
					}
			...
			switch($aSet['type']) {
				...
				case "jpg": 
					...
					if(empty($aSet['quality'])) {
						$aSet['quality'] = 75;
					}
					
					if(function_exists('imagejpeg')) {
						if($bolSave) {
							imagejpeg($rImg, $strTargetPath, $aSet['quality']);
						} else {
							header('Content-type: image/jpeg');
							imagejpeg($rImg, null, $aSet['quality']);
						}
					}		
					break;
				case "png": 
					...
					if(function_exists('imagepng')) {
						if($bolSave) {
							imagepng($rImg, $strTargetPath, $aSet['quality']);
						} else {
							header('Content-type: image/png');
							imagepng($rImg, null, $aSet['quality']);
						}
					}		
					break;
			}
			// set rights
			if($bolSave) {
				Util::changeFileMode($strTargetPath);
			}
			imagedestroy($rImg);
		}
    ...
    */
    function imageMask($bg = 'FFFFFF')
    {
        ...
        $this->_img     = array();
        $this->_mask    = array();
        $this->_bgc     = self::_htmlHexToBinArray($bg);
    }
    ...
    */
    public static function _htmlHexToBinArray($hex) {
	...
	public static function createThumbnail($sOriginal, $sThumb, $new_width, $new_height) {

		$mime = getimagesize($sOriginal);

		if($mime['mime']=='image/png'){ $src_img = imagecreatefrompng($sOriginal); }
		if($mime['mime']=='image/jpg'){ $src_img = imagecreatefromjpeg($sOriginal); }
		if($mime['mime']=='image/jpeg'){ $src_img = imagecreatefromjpeg($sOriginal); }
		if($mime['mime']=='image/pjpeg'){ $src_img = imagecreatefromjpeg($sOriginal); }
		...
		if($mime['mime']=='image/png'){ $result = imagepng($dst_img,$sThumb,8); }
		if($mime['mime']=='image/jpg'){ $result = imagejpeg($dst_img,$sThumb,80); }
		if($mime['mime']=='image/jpeg'){ $result = imagejpeg($dst_img,$sThumb,80); }
		if($mime['mime']=='image/pjpeg'){ $result = imagejpeg($dst_img,$sThumb,80); }
	...
	static public function getImgBuilderPath($setId, $originalImage) {
		
		$originalImage = ltrim($originalImage, '/');

		$imageBuilder = new \imgBuilder();
		
		$imageBuilderInfo = [0=>0];
		$imageBuilderInfo[1] = (int)$setId;
		$imageBuilderInfo[2] = $originalImage;

		$image = $imageBuilder->buildImage($imageBuilderInfo);

		$pathInfo = pathinfo(\Util::getDocumentRoot(false).$image);

		$pathInfoOriginal = pathinfo($imageBuilder->strImagePath.$originalImage);
