 ...
 class WDChart
  {
   ...
   public function __construct($XSize,$YSize) {
   		...
   		$this->XSize   = $XSize;
		$this->YSize   = $YSize;
		$this->rPicture = imagecreatetruecolor($XSize,$YSize);

		$C_White =$this->AllocateColor($this->rPicture,255,255,255);
		imagefilledrectangle($this->rPicture,0,0,$XSize,$YSize,$C_White);
		imagecolortransparent($this->rPicture,$C_White);
  ...
  function reportWarnings($Interface="CLI")
   ...
   function setFontProperties($FontName,$FontSize)
    {
     $this->FontName = \Util::getDocumentRoot().'system/includes/wdchart/fonts/'.$FontName;
   ...
   function loadColorScheme($FileName, $Delimiter=",")
    {

    	$FileName = \Util::getDocumentRoot().'system/includes/wdchart/colorschemes/'.$FileName.'.txt';
     ...
     if ( $Stripe )
      {
       ...
       $LineColor =$this->AllocateColor($this->rPicture,$R2,$G2,$B2);
...
// * Fixed in 1.27 *         { $X2 = $this->GArea_X2 - 1; $Y2 = $this->GArea_Y2 - ($this->GArea_X2 - $X1); }
         ...
         imageline($this->rPicture,$X1,$Y1,$X2,$Y2+1,$LineColor);
        }
      }
    }
   ...
   function drawScale($Data,$DataDescription,$ScaleMode,$R,$G,$B,$DrawTicks=TRUE,$Angle=0,$Decimals=1,$WithMargin=FALSE,$SkipLabels=1,$RightScale=FALSE)
    {
     ...
     $C_TextColor         =$this->AllocateColor($this->rPicture,$R,$G,$B);
     ...
     for($i=1;$i<=$Divisions+1;$i++)
      {
       ...
       if ( $RightScale )
        {
         imagettftext($this->rPicture,$this->FontSize,0,$this->GArea_X2+10,$YPos+($this->FontSize/2),$C_TextColor,$this->FontName,$Value);
         if ( $XMin < $this->GArea_X2+15+$TextWidth || $XMin == NULL ) { $XMin = $this->GArea_X2+15+$TextWidth; }
        }
       else
        {
         imagettftext($this->rPicture,$this->FontSize,0,$this->GArea_X1-10-$TextWidth,$YPos+($this->FontSize/2),$C_TextColor,$this->FontName,$Value);
     ...
     if ( isset($DataDescription["Axis"]["Y"]) )
      {
       ...
       if ( $RightScale )
        imagettftext($this->rPicture,$this->FontSize,90,$XMin+$this->FontSize,$TextTop,$C_TextColor,$this->FontName,$DataDescription["Axis"]["Y"]);
       else
        imagettftext($this->rPicture,$this->FontSize,90,$XMin-$this->FontSize,$TextTop,$C_TextColor,$this->FontName,$DataDescription["Axis"]["Y"]);
      }

		// Imgbuilder für erweiterten Text einstanzieren
		$oImgBuilder = new imgBuilder();
		$oImgBuilder->strFontPath = '';
         ...
         if ( $Angle == 0 )
          {
           ...
           //imagettftext($this->rPicture,$this->FontSize,$Angle,floor($XPos)-floor($TextWidth/2),$YPos,$C_TextColor,$this->FontName,$Value);

		   $iTextWidth = $this->DivisionWidth-4;

		   $aText = $oImgBuilder->determineWordWrap($Value, $iTextWidth, 0, $this->FontSize, $this->FontName);

		   $oImgBuilder->printTTFText($this->rPicture, $this->FontSize, $Angle, (floor($XPos) - $this->DivisionWidth/2 + 2), $YPos, $C_TextColor, $this->FontName, $aText['text'], $iTextWidth, 'C', 0, $aText['lineheight']);
         ...
         else
          {
           ...
           if ( $Angle <= 90 )
            imagettftext($this->rPicture,$this->FontSize,$Angle,floor($XPos)-$TextWidth+5,$YPos,$C_TextColor,$this->FontName,$Value);
           else
            imagettftext($this->rPicture,$this->FontSize,$Angle,floor($XPos)+$TextWidth+5,$YPos,$C_TextColor,$this->FontName,$Value);
          }
    ...
    if ( isset($DataDescription["Axis"]["X"]) )
      {
       ...
       imagettftext($this->rPicture,$this->FontSize,0,$TextLeft,$YMax+$this->FontSize+5,$C_TextColor,$this->FontName,$DataDescription["Axis"]["X"]);
      }
    }
     ...
     if ( $Mosaic )
      {
       ...
       imagecopymerge($this->rPicture,$this->Layers[0],$this->GArea_X1,$this->GArea_Y1,0,0,$LayerWidth,$LayerHeight,$Alpha);
   ...
   function drawLegend($XPos,$YPos,$DataDescription,$R,$G,$B,$Rs=-1,$Gs=-1,$Bs=-1,$Rt=0,$Gt=0,$Bt=0,$Border=TRUE)
    {
     ...
     $C_TextColor =$this->AllocateColor($this->rPicture,$Rt,$Gt,$Bt);
     ...
     foreach($DataDescription["Description"] as $Key => $Value)
      {
       ...
       imagettftext($this->rPicture,$this->FontSize,0,$XPos+22,$YPos+$YOffset,$C_TextColor,$this->FontName,$Value);
	...
	public function drawPieLegend($XPos, $YPos, $Data, $DataDescription, $R, $G, $B) {
		...
		$C_TextColor =$this->AllocateColor($this->rPicture,0,0,0);

		$aDimensions = imagettfbbox($this->FontSize, 0, $this->FontName, $this->strCheckString);
		$iTextHeight = abs($aDimensions[5] - $aDimensions[1]);
		...
		foreach($Data as $Key => $Value) {
	       ...
	       $MaxHeight = $MaxHeight + $iTextHeight + round($iTextHeight/3, 0);
		}
		...
		$YOffset = round($iTextHeight/3, 0) + $this->FontSize; 
		$ID = 0;
		foreach($Data as $Key => $Value) {
	       $Value2     = $Value[$DataDescription["Position"]];
	       ...
	       imagettftext($this->rPicture, $this->FontSize, 0, $XPos+22, $YPos+$YOffset, $C_TextColor, $this->FontName, $Value2);
	       $YOffset = $YOffset + $iTextHeight + round($iTextHeight/3, 0);
	       $ID++;
		}
   ...
   function drawTitle($XPos, $YPos, $Value, $R, $G, $B, $XPos2=-1, $YPos2=-1, $Shadow=FALSE)
    {
     ...
     if ( $Shadow )
      {
       $C_ShadowColor = $this->AllocateColor($this->rPicture,$this->ShadowRColor,$this->ShadowGColor,$this->ShadowBColor);
       imagettftext($this->rPicture,$this->FontSize,0,$XPos+$this->ShadowXDistance,$YPos+$this->ShadowYDistance,$C_ShadowColor,$this->FontName,$Value);     
      }

     imagettftext($this->rPicture,$this->FontSize,0,$XPos,$YPos,$C_TextColor,$this->FontName,$Value);     
    }
   ...
   function drawTextBox($X1,$Y1,$X2,$Y2,$Text,$Angle=0,$R=255,$G=255,$B=255,$Align=ALIGN_LEFT,$Shadow=TRUE,$BgR=-1,$BgG=-1,$BgB=-1,$Alpha=100)
    {
     ...
     $C_TextColor   =$this->AllocateColor($this->rPicture,$R,$G,$B);
     $C_ShadowColor =$this->AllocateColor($this->rPicture,0,0,0);
     if ( $Shadow )
      imagettftext($this->rPicture,$this->FontSize,$Angle,$X+1,$Y+1,$C_ShadowColor,$this->FontName,$Text);     

     imagettftext($this->rPicture,$this->FontSize,$Angle,$X,$Y,$C_TextColor,$this->FontName,$Text);     
    }
     ...
     if ( $ShowLabel )
      {
       ...
       if ( $ShowOnRight )
        imagettftext($this->rPicture,$this->FontSize,0,$this->GArea_X2+2,$Y+($this->FontSize/2),$C_TextColor,$this->FontName,$Label);
       else
        imagettftext($this->rPicture,$this->FontSize,0,$this->GArea_X1+2,$Y-($this->FontSize/2),$C_TextColor,$this->FontName,$Label);
      }
    }
   ...
   function setLabel($Data,$DataDescription,$SerieName,$ValueName,$Caption,$R=210,$G=210,$B=210)
    {
     ...
     $ShadowFactor = 100;
     $C_Label      =$this->AllocateColor($this->rPicture,$R,$G,$B);
     $C_Shadow     =$this->AllocateColor($this->rPicture,$R-$ShadowFactor,$G-$ShadowFactor,$B-$ShadowFactor);
     $C_TextColor  =$this->AllocateColor($this->rPicture,0,0,0);
     ...
     imagefilledpolygon($this->rPicture,$Poly,3,$C_Shadow);
     ...
     imagefilledpolygon($this->rPicture,$Poly,3,$C_Label);
     ...
     imagettftext($this->rPicture,$this->FontSize,0,$XPos + 10,$YPos + $TextOffset,$C_TextColor,$this->FontName,$Caption);
    }
         ...
         if ( is_numeric($Value) )
          {
           if ( !isset($DataDescription["Symbol"][$ColName]) )
            ...
            {
             imagecopymerge($this->rPicture,$Symbol,$XPos+1-$ImageWidth/2,$YPos+1-$ImageHeight/2,0,0,$ImageWidth,$ImageHeight,100);
            }
          }
   ...
   function drawArea($Data,$Serie1,$Serie2,$R,$G,$B,$Alpha = 50)
	 ...
	 * @param type $aData
	 * @param type $aDataDescription
	 * @param type $aSeries 
	 */
	function writeValues($aData, $aDataDescription, $aSeries) {
		
		/* Validate the Data and DataDescription array */
		$this->validateDataDescription("writeValues",$aDataDescription);
		$this->validateData("writeValues",$aData);

		if ( !is_array($aSeries) ) { 
			$aSeries = array($aSeries); 
		}     

		$this->_sChartType;
		
		$iVMin = $this->VMin * $this->DivisionRatio;
		
		$iSeries       = count($aDataDescription["Values"]);
		$iSeriesWidth  = round((int)$this->DivisionWidth / ($iSeries+1));
		$iSerieXOffset = round($iSeriesWidth / 2);                      
                                      
		$aLastValue = array();                                          
		foreach($aSeries as $iSerieKey => $sSerie) {                    
                                                                        
			$ID = 0;                                                    
			foreach ( $aDataDescription["Description"] as $keyI => $ValueI ) { 
				if ( $keyI == $sSerie ) {                               
					$ColorID = $ID;                                     
					break;                                              
				};                                                      
			...
			$XPos  = $this->GArea_X1 + $iSerieXOffset + ($iSeriesWidth * $ColorID) + ($iSeriesWidth / 2);
			$XLast = -1;                                                

			foreach ( $aData as $iKey => $Values ) {

				if ( 
					isset($aData[$iKey][$sSerie]) && 
					is_numeric($aData[$iKey][$sSerie])
				) {

					$iValue = $aData[$iKey][$sSerie];
					$iValueHeight = $iValue * $this->DivisionRatio;

					$Positions = imagettfbbox($this->FontSize, 0, $this->FontName, $iValue);
					$Width  = $Positions[2] - $Positions[6]; 
					$Height = $Positions[3] - $Positions[7]; 
					
					if($this->_sChartType == 'stacked_bar') {
						if(isset($aLastValue[$iKey])) {
							$iYOffset = $aLastValue[$iKey];
							$aLastValue[$iKey] += $iValueHeight;
						} else {
							$aLastValue[$iKey] = $iValueHeight;
							$iYOffset = $aLastValue[$iKey] - ($Height*2) + 4;
						}
						$YPos = $this->GArea_Y2 - ($iYOffset - $iVMin);
					} else {
						$YPos = $this->GArea_Y2 - ($iValueHeight - $iVMin);
					}
					...
					if($this->_sChartType == 'stacked_bar') {
						if($ColorID == 0) {
							$aColor = $this->Palette[($ColorID+1)];
						} else {
							$aColor = $this->Palette[($ColorID-1)];
						}
					} else {
						$aColor = $this->Palette[$ColorID];
					}

					$C_TextColor =$this->AllocateColor($this->rPicture, $aColor["R"], $aColor["G"], $aColor["B"]);
					imagettftext($this->rPicture, $this->FontSize, 0, $XOffset, $YOffset, $C_TextColor, $this->FontName, $iValue);
   ...
   function drawOverlayBarGraph($Data,$DataDescription,$Alpha=50)
    {
     ...
     for($i=0;$i<=($GraphID-1);$i++)
      {
       imagecopymerge($this->rPicture,$this->Layers[$i],$this->GArea_X1,$this->GArea_Y1,0,0,$LayerWidth,$LayerHeight,$Alpha);
   ...
   function drawBarGraph($Data,$DataDescription,$Shadow=FALSE,$Alpha=100)
    {
     ...
     if ( $YZero > $this->GArea_Y2 ) { $YZero = $this->GArea_Y2 - 1; }
   ...
   function drawRadarAxis($Data,$DataDescription,$Mosaic=TRUE,$BorderOffset=10,$A_R=60,$A_G=60,$A_B=60,$S_R=200,$S_G=200,$S_B=200,$MaxValue=-1)
    {
     ...
     $C_TextColor = $this->AllocateColor($this->rPicture,$A_R,$A_G,$A_B);
           ...
           if ( $t % 2 == 1 && $LastX1 != -1)
            {
             ...
             $C_Graph = $this->AllocateColor($this->rPicture,250,250,250);
             imagefilledpolygon($this->rPicture,$Plots,(count($Plots)+1)/2,$C_Graph);
            }
       ...
       if (isset($Data[$i][$DataDescription["Position"]]))
        {
         ...
         imagettftext($this->rPicture,$this->FontSize,0,$X+$XOffset,$Y+$YOffset,$C_TextColor,$this->FontName,$Label);
        }
      }
     ...
     for ( $t=1; $t<=$MaxValue; $t++)
      {
       ...
       imagettftext($this->rPicture,$this->FontSize,0,$X,$Y,$C_TextColor,$this->FontName,$t);
      }
    }
   ...
   function drawBasicPieGraph($Data, $DataDescription, $XPos, $YPos, $Radius=100, $DrawLabels=PIE_NOLABEL, $R=255, $G=255, $B=255, $Decimals=0)
    {
       ...
       if ( !($DrawLabels == PIE_NOLABEL) )
        {
         ...
         $C_TextColor = $this->AllocateColor($this->rPicture,70,70,70);
         imagettftext($this->rPicture,$this->FontSize,0,$TX,$TY,$C_TextColor,$this->FontName,$Caption);
        }
     ...
     foreach ($PolyPlots as $Key => $Value)
      { 
       $C_GraphLo = $this->AllocateColor($this->rPicture,$this->Palette[$Key]["R"],$this->Palette[$Key]["G"],$this->Palette[$Key]["B"]);
       imagefilledpolygon($this->rPicture,$PolyPlots[$Key],(count($PolyPlots[$Key])+1)/2,$C_GraphLo);
      }
     ...
     foreach($iValues as $Key => $Value)
      {
       ...
       /* Process pie slices */
       if ( !$AllBlack )
        $LineColor = $this->AllocateColor($this->rPicture,$Rc,$Gc,$Bc);
       else
        $LineColor = $this->AllocateColor($this->rPicture,$Rc,$Gc,$Bc);
     ...
     foreach ($PolyPlots as $Key => $Value)
      { 
       if ( !$AllBlack )
        ...
        $C_GraphLo = $this->AllocateColor($this->rPicture,$this->ShadowRColor,$this->ShadowGColor,$this->ShadowBColor);
   ...
   function drawPieGraph($Data,$DataDescription,$XPos,$YPos,$Radius=100,$DrawLabels=PIE_NOLABEL,$EnhanceColors=TRUE,$Skew=60,$SpliceHeight=20,$SpliceDistance=0,$Decimals=0)
    {
     ...
     foreach($iValues as $Key => $Value)
      {
       $C_GraphLo = $this->AllocateColor($this->rPicture,$this->Palette[$Key]["R"],$this->Palette[$Key]["G"],$this->Palette[$Key]["B"],-20);
       imagefilledpolygon($this->rPicture,$BotPlots[$Key],(count($BotPlots[$Key])+1)/2,$C_GraphLo);
     ...
     for($i=$SpliceHeight-1;$i>=1;$i--)
      {
       foreach($iValues as $Key => $Value)
        {
         $C_GraphLo = $this->AllocateColor($this->rPicture,$this->Palette[$Key]["R"],$this->Palette[$Key]["G"],$this->Palette[$Key]["B"],-10);
         $Plots = ""; $Plot = 0;
         ...
         imagefilledpolygon($this->rPicture,$Plots,(count($Plots)+1)/2,$C_GraphLo);
     ...
     for($Key=count($iValues)-1;$Key>=0;$Key--)
      { 
       $C_GraphLo = $this->AllocateColor($this->rPicture,$this->Palette[$Key]["R"],$this->Palette[$Key]["G"],$this->Palette[$Key]["B"]);
       imagefilledpolygon($this->rPicture,$TopPlots[$Key],(count($TopPlots[$Key])+1)/2,$C_GraphLo);
   ...
   function drawBackground($R,$G,$B)
    {
     ...
     $C_Background = $this->AllocateColor($this->rPicture,$R,$G,$B);
     imagefilledrectangle($this->rPicture,0,0,$this->XSize,$this->YSize,$C_Background);
    }
       ...
       for($i=0;$i<=$Decay;$i++)
        {
         ...
         $C_Background = $this->AllocateColor($this->rPicture,$R,$G,$B);
         imagefilledrectangle($this->rPicture,$X1,$Yi1,$X2,$Yi2,$C_Background);
        }
      }
   ...
   function drawRectangle($X1,$Y1,$X2,$Y2,$R,$G,$B)
    {
     ...
     $C_Rectangle = $this->AllocateColor($this->rPicture,$R,$G,$B);
     ...
     if ( $Alpha == 100 )
      {
       ...
       $C_Rectangle = $this->AllocateColor($this->rPicture,$R,$G,$B);
       imagefilledrectangle($this->rPicture,round($X1),round($Y1),round($X2),round($Y2),$C_Rectangle);
      }
     else
      {
       ...
       imagecopymerge($this->rPicture,$this->Layers[0],round(min($X1,$X2)-1),round(min($Y1,$Y2)-1),0,0,$LayerWidth,$LayerHeight,$Alpha);
       imagedestroy($this->Layers[0]);
      }
   ...
   function drawFilledRoundedRectangle($X1,$Y1,$X2,$Y2,$Radius,$R,$G,$B)
    {
     ...
     for($i=0;$i<=90;$i=$i+$Step)
      {
       ...
       imageline($this->rPicture,$Xi1,$Yi1,$X1+$Radius,$Yi1,$C_Rectangle);
       imageline($this->rPicture,$X2-$Radius,$Yi2,$Xi2,$Yi2,$C_Rectangle);
       imageline($this->rPicture,$X2-$Radius,$Yi3,$Xi3,$Yi3,$C_Rectangle);
       imageline($this->rPicture,$Xi4,$Yi4,$X1+$Radius,$Yi4,$C_Rectangle);
     ...
     imagefilledrectangle($this->rPicture,$X1,$Y1+$Radius,$X2,$Y2-$Radius,$C_Rectangle);
     imagefilledrectangle($this->rPicture,$X1+$Radius,$Y1,$X2-$Radius,$Y2,$C_Rectangle);
   ...
   function drawCircle($Xc,$Yc,$Height,$R,$G,$B,$Width=0)
    {
     ...
     $C_Circle = $this->AllocateColor($this->rPicture,$R,$G,$B);
     ...
     for($i=90;$i<=270;$i=$i+$Step)
      {
       ...
       if ( ($Y1-1) > $Yc - max($Width,$Height) )
        imageline($this->rPicture,$X1,$Y1-1,$X2-1,$Y2-1,$C_Circle);
      }
    }
     ...
     if ( file_exists($FileName))
      {
       ...
       imagecopymerge($this->rPicture,$Raster,$X,$Y,0,0,$Width,$Height,$Alpha);
       imagedestroy($Raster);
      }
    }
   ...
   function drawAlphaPixel($X,$Y,$Alpha,$R,$G,$B)
    {
     ...
     $RGB2 = imagecolorat($this->rPicture, $X, $Y);
     $R2   = ($RGB2 >> 16) & 0xFF;
     $G2   = ($RGB2 >> 8) & 0xFF;
     $B2   = $RGB2 & 0xFF;
     ...
     $C_Aliased = $this->AllocateColor($this->rPicture,$Ra,$Ga,$Ba);
     imagesetpixel($this->rPicture,$X,$Y,$C_Aliased);
    }
   ...
   function addBorder($Size=3,$R=0,$G=0,$B=0)
    {
     ...
     imagecopy($Resampled,$this->rPicture,$Size,$Size,0,0,$this->XSize,$this->YSize);
     imagedestroy($this->rPicture);

     $this->XSize = $Width;
     $this->YSize = $Height;

     $this->rPicture = imagecreatetruecolor($this->XSize,$this->YSize);
     $C_White = $this->AllocateColor($this->rPicture,255,255,255);
     imagefilledrectangle($this->rPicture,0,0,$this->XSize,$this->YSize,$C_White);
     imagecolortransparent($this->rPicture,$C_White);
     imagecopy($this->rPicture,$Resampled,0,0,0,0,$this->XSize,$this->YSize);
    }
   ...
   function save($FileName)
    {
     ...
     imagepng($this->rPicture, $FileName);
    }

    public function output() {
    	header('Content-type: image/png');
		imagepng($this->rPicture);
    }
     ...
     if ( $Xi == $X && $Yi == $Y)
      {
       if ( $Alpha == 100 )
        {
         $C_Aliased = $this->AllocateColor($this->rPicture,$R,$G,$B);
         imagesetpixel($this->rPicture,$X,$Y,$C_Aliased);
        }
       else
     ...
     elseif ( $Mode == "GD" )
      {
       ...
       $C_TextColor = $this->AllocateColor($this->rPicture,133,85,85);
       $YPos        = $this->YSize - (18 + (count($this->Errors)-1) * ($this->ErrorFontSize + 4));
       foreach($this->Errors as $key => $Value)
        {
         imagettftext($this->rPicture,$this->ErrorFontSize,0,$this->XSize-($MaxWidth+15),$YPos,$C_TextColor,$this->ErrorFontName,$Value);
