    ...
    $.extend(Fastselect.prototype, {
        ...
        processInitialOptions: function() {
            ...
            if (this.hasCustomLoader) {
				...
				var initialValue = this.$input[0].value;

				initialValues = initialValue.split(this.options.valueDelimiter);

				initialOptions = [];
				options && $.each(options, function(i, option) {
					if(initialValues.indexOf(option.value) != -1) {
						self.optionsCollection.setSelected(option);
						initialOptions.push(option);
					}
				});

				$.isPlainObject(initialOptions) && (initialOptions = [initialOptions]);
