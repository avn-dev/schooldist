			...
			if(
				...
				// START Hidden Feld zum überschreibe von Schüler- und Buchungskontaktdaten
					if($('dialog_form_'+aData.id+'_'+this.hash)){
                        ...
                        var replaceCustomerId = 'replaceCustomerId['+this.hash+']['+aData.id+']';
                        var replaceBookerId = 'replaceBookerId['+this.hash+']['+aData.id+']';
                        var replaceHubspotContactId = 'replaceHubspotContactId['+this.hash+']['+aData.id+']';

                        var ensureHiddenField = function(fieldId, fieldName) {
                            var existingFields = oForm.select('input[id="' + fieldId + '"]');

                        if(existingFields.length){
                            existingFields[0].value = 0;

                            if(existingFields.length > 1){
                                existingFields.slice(1).invoke('remove');
                            }
                        }else{
                            oForm.insert({
                ...
                ensureHiddenField(replaceCustomerId, 'replaceCustomerId');
                ensureHiddenField(replaceBookerId, 'replaceBookerId');
                ensureHiddenField(replaceHubspotContactId, 'replaceHubspotContactId');
					}
				// ENDE
		...
		$j.each(properties, function(sKey, mValue) {
			...
			switch(sKey) {
				case 'date_of_birth':
					...
					break;
				case 'email':
					$j('#container_contact_email').find('.remove_icon').trigger('click', true);
