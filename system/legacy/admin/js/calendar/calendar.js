...
{
	var strRequestUrl = '/admin/js/calendar/calendar.ajax.php';
	var strParameters = 'task=load_names';

	$H(aParams).each(function(oParam) {
		strParameters += '&' + oParam.key + '=' + encodeURIComponent(oParam.value);
	});
	...
	try
	{
		var objAjax = new Ajax.Request(
								strRequestUrl,
								{
									...
									onComplete	: Calendar.prepareCallback
								}
		);
	}
	catch(e)
...
Calendar.prepareCallback = function(oResponse)
{
	...
	Calendar.setup({
		dateField     	: aData['aParams']['dateField'],
		triggerElement	: aData['aParams']['triggerElement'],
		parentElement	: aData['aParams']['parentElement'],
		selectHandler	: eval(aData['aParams']['selectHandler']),
		closeHandler	: eval(aData['aParams']['closeHandler']),
		...
		extcloseHandler : aData['aParams']['extcloseHandler']
	});
...
Calendar.prototype = {
  ...
  update: function(date)
  {
    ...
    date.setDate(1)
    //date.setDate(-(date.getDay()) + 2); // TODO: Das ist dar Richtige:
    date.setDate(-(date.getDay() + 7) + 2);

	if(date.getMonthDays() >= (date.getDate() + 7))
	{
...
//    for (var i = 0; i < 7; ++i) {
	for (var i = 1; i <= 7; ++i) {
      cell = new Element('th').update(this.shortDayNames[i])
      if (i == 0 || i == 6)
        cell.addClassName('weekend')
      row.appendChild(cell)
    }
  ...
  showAtElement: function(element) {
    ...
    var iHeight = element.getHeight();
	var pos = element.cumulativeOffset();
	var posScroll = this.cumulativeScrollOffset(element);
	var Top = 0;
	var Left = 0;
	var iWindowHeight = window.innerHeight || document.body.offsetHeight;
	var iCalendarHeight = this.container.getHeight();

	var aBodyScroll = this.getBodyScroll();

	if(aBodyScroll[1] && aBodyScroll[1] != 0)
	{
		Top += aBodyScroll[1];
	}
	if(aBodyScroll[0] && aBodyScroll[0] != 0)
	{
		Left += aBodyScroll[0];
	}

	Left 	+= parseFloat(pos[0]) - parseFloat(posScroll[0]);
	Top 	+= parseFloat(pos[1]) - parseFloat(posScroll[1]) + iHeight + 5;

	// Prüfen ob Kalender zu groß ist für die Seite, dann noch oben spiegeln
	if((aBodyScroll[1] + iWindowHeight) <= (Top + iCalendarHeight)){
		Top = Top - iCalendarHeight - iHeight - 10;
	}
	...
	getBodyScroll: function() {
		...
		var iThisT=0, iThisL=0;

		var bodyScrollNode = document.documentElement ||
		  document.body.parentNode || document.body;
		iThisT = !Object.isUndefined(window.pageYOffset) ?
		  window.pageYOffset : bodyScrollNode.scrollTop || 0;
		iThisL = !Object.isUndefined(window.pageXOffset) ?
		  window.pageXOffset : bodyScrollNode.scrollLeft || 0;

		return [iThisL, iThisT];

	},
	
  cumulativeScrollOffset: function(element) {
	var valueT = 0, valueL = 0, iThisT=0, iThisL=0;
	do {
	
	  if (element === document.body) {
		var aScroll = this.getBodyScroll();
		iThisL = aScroll[0];
		iThisT = aScroll[1];
	  } else {
	    iThisT = element.scrollTop || 0;
	    iThisL = element.scrollLeft || 0;
	  }
	...
	var mReturn = Element._returnOffset(valueL, valueT);
