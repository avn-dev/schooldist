...
function tableEditor(docID, pntCell) {
   ...
   if (typeof(_tableEditor_prototype_called) == 'undefined') {
	  ...
	  tableEditor.prototype.__showTableIcon = __showTableIcon;
	  tableEditor.prototype.__hideTableIcon = __hideTableIcon;
   }
   ...
   function setTableElements(){
      ...
      cursorPos=parent.content.page.document.selection.createRange();
      if (parent.content.page.document.selection.type == 'Text') {
         ...
         if (cursorPos.length == 1) {
            if (cursorPos.item(0).tagName == "TABLE") {
               ...
               this.cellWidth = this.tableElem.offsetWidth;
			   this.cellHeight = this.tableElem.offsetHeight;
			   
			   this.__showTableIcon();
               this.__hideArrows();
               this.tableCell = null;
            }
         }
   ...
   function unMergeDown() {
      ...
      this.tableCell.parentNode.parentNode.childNodes[ topRowIndex + this.tableCell.rowSpan - 1 ].appendChild( parent.content.page.document.createElement("TD") );
   ...
   function splitCell() {
      ...
      var newCell = this.tableCell.parentNode.insertBefore( parent.content.page.document.createElement("TD"), this.tableCell);
   ...
   function addCell() {
      ...
      this.tableCell.parentElement.insertBefore(parent.content.page.document.createElement("TD"), this.tableCell.nextSibling);
   }
   ...
   function processRow(action) {
      ...
      if (action == "add") {
         ...
         for (var i = 0; i < numcells; i++) {
            var c = r.appendChild( parent.content.page.document.createElement("TD") );
   ...
   function __addOrRemoveCols(tbl, cellidx, action) {
      ...
      for (i = 0; i < tbl.childNodes.length; i++) {
         if (tbl.childNodes[i].tagName == "TR") {
            ...
            if (action == "add") {
               cell.insertAdjacentElement("AfterEnd",  parent.content.page.document.createElement("TD") );
            } else {
   ...
   function exploreTree(obj, pnt) {
      if (!obj.childNodes.length)
         return;
      var i;
      var ul = pnt.appendChild( parent.content.page.document.createElement("UL") );
      for (i = 0; i < obj.childNodes.length; i++) {
         var li = parent.content.page.document.createElement("LI");
   ...
   function explore(obj, pnt) {
      var i;
      for (i in obj) {
         var n = parent.content.page.document.createTextNode(i +"="+obj[i]);
         pnt.appendChild(n);
         pnt.appendChild( parent.content.page.document.createElement("BR") );
      }
   }
   ...
   function buildTable(pnt) {
      var t = pnt.appendChild( parent.content.page.document.createElement("TABLE") );
      t.border=1;
      t.cellPadding=2;
      t.cellSpacing=0;
      var tb = t.appendChild( parent.content.page.document.createElement("TBODY") );
      for(var r = 0; r < 10; r++) {
         var tr = tb.appendChild( parent.content.page.document.createElement("TR") );
         for(var c = 0; c < 10; c++) {
            var cell = tr.appendChild( parent.content.page.document.createElement("TD") );
            cell.appendChild( parent.content.page.document.createTextNode(r+"-"+c) );
         }
      }
   }
   ...
   function changePos() {
      ...
      this.moveable.style.posTop = parent.content.page.event.clientY - 10;
      this.moveable.style.posLeft = parent.content.page.event.clientX - 25;
   }
   ...
   function __hideArrows() {
      parent.content.page.document.getElementById("rArrow").style.visibility = 'hidden';
      parent.content.page.document.getElementById("dArrow").style.visibility = 'hidden';
   }

   function __hideTableIcon() {
      parent.content.page.document.getElementById("rTable").style.visibility = 'hidden';
   }
   ...
   function __showArrows() {
      ...
      var scrollTop = parent.content.page.document.getElementById(this.docID).scrollTop;
      var scrollLeft = parent.content.page.document.getElementById(this.docID).scrollLeft;

      parent.content.page.document.getElementById("rArrow").style.posLeft = cell_wdt + this.tableCell.offsetWidth - 6 - scrollLeft;
      parent.content.page.document.getElementById("rArrow").style.posTop = cell_hgt + (this.tableCell.offsetHeight / 2) - 2 - scrollTop;

      parent.content.page.document.getElementById("dArrow").style.posLeft = cell_wdt + (this.tableCell.offsetWidth / 2) - 2 - scrollLeft;
      parent.content.page.document.getElementById("dArrow").style.posTop = cell_hgt + this.tableCell.offsetHeight - 6 - scrollTop;

      parent.content.page.document.getElementById("rArrow").style.visibility = 'visible';
      parent.content.page.document.getElementById("dArrow").style.visibility = 'visible';
   }

   function __showTableIcon() {
      if (!this.tableElem)
         return;

      var cell_hgt = this.tableElem.offsetTop;
      var cell_wdt = this.tableElem.offsetLeft;
      var par = this.tableElem.offsetParent;
      while (par) {
         cell_hgt = cell_hgt + par.offsetTop;
         cell_wdt = cell_wdt + par.offsetLeft;
         current_obj = par;
      ...
      this.cellX = cell_wdt + this.tableElem.offsetWidth; //bottom right X
      this.cellY = cell_hgt + this.tableElem.offsetHeight; // bottom right Y

      var scrollTop = parent.content.page.document.getElementById(this.docID).scrollTop;
      var scrollLeft = parent.content.page.document.getElementById(this.docID).scrollLeft;
	  
      parent.content.page.document.getElementById("rTable").style.posLeft = cell_wdt + this.tableElem.offsetWidth - 20 - scrollLeft;
      parent.content.page.document.getElementById("rTable").style.posTop = cell_hgt + (this.tableElem.offsetHeight) - 20 - scrollTop;

      parent.content.page.document.getElementById("rTable").style.visibility = 'visible';
   }
   ...
   function resizeCell() {
      ...
      if (this.cellResizeObj.id == 'dArrow') {
         var scrollTop = parent.content.page.document.getElementById(this.docID).scrollTop;
         var newHeight = (parent.content.page.event.clientY - (this.cellY - scrollTop) ) + this.cellHeight;
         ...
         var scrollLeft = parent.content.page.document.getElementById(this.docID).scrollLeft;
         var newWidth = (parent.content.page.event.clientX - (this.cellX - scrollLeft) ) + this.cellWidth;
...
function c_table( e, v ) {
	var html = parent.content.page.showModalDialog( '/admin/ins_table.html', v, 'scroll:0;help:0;status:0;dialogWidth:315px;dialogHeight:350px' );

	if(e == "new") {
		parent.content.page.document.getElementById(tEdit.docID).focus();
		var sel = parent.content.page.document.selection.createRange();
        sel.pasteHTML(html);
	}				
		toggle_borders();
...
function handlemousedblclick( ev ) {
  if( hOnMouseDblClick.length ) eval( hOnMouseDblClick );
}

function dblclickfkt() {
	var e = parent.content.page.event.srcElement;
	switch( e.tagName ) {
		case 'TABLE':
			c_table( null, e );
			break;
