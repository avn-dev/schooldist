...
class Thebing_Snippet {
	...
	protected $_bSameSiteCookie = false;
	...
	public function enableSameSiteCookie() {
		$this->_bSameSiteCookie = true;
	}
...
//			}
			...
			if(is_scalar($mValue)) {
				if(
					...
					try {
						if ($this->_bSameSiteCookie) {
							setrawcookie($sKey, $mValue, 0, '/; secure; SameSite=none');  //setrawcookie encodiert den Cookie nicht. (12;34 = 12) Eingebaut für Looks Cookie Consent Tool.
						} else {
							setrawcookie($sKey, $mValue, 0, '/');  //setrawcookie encodiert den Cookie nicht. (12;34 = 12) Eingebaut für Looks Cookie Consent Tool.
						}
					} catch(\Throwable $e) {
						// Seit PHP8-Version wird scheinbar ein Fatal error geschmissen wenn der Inhalt ein invalides Zeichen enthält (z.b. json aus 'browsercookie')
