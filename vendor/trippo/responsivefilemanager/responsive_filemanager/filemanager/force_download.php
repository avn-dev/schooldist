...
if (!checkRelativePath($_POST['path']) || strpos($_POST['path'], '/') === 0) {
    response(t_rf_rans('wrong path') . AddErrorLocation(), 400)->send();
    exit;
}
...
if (!check_extension($info['extension'], $config)) {
    response(t_rf_rans('wrong extension') . AddErrorLocation(), 400)->send();
    exit;
}
...
if ($ftp) {
    ...
    if (!file_exists($path . $name)) {
        response(t_rf_rans('File_Not_Found') . AddErrorLocation(), 404)->send();
        exit;
    }
