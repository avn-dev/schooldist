...
#                to turn t_rf_ransparency off (like when saving
#                as a jpg).
#              * Added setFillColor() set method - set the
#                background color to use instead of t_rf_ransparency.
...
#  Tips:
# -------------------------------
#  * If you're resizing a t_rf_ransparent png and saving it as a jpg, set
...
#    * RESIZE IMAGES
#      - Preserve t_rf_ransparency (png, gif)
...
class imageLib {
	...
	private $t_rf_ransparentArray = array( '.png', '.gif' );
...
## --------------------------------------------------------
	...
	private function keepTransparancy($width, $height, $im)
		# Author:     Jarrod Oberto
		# Date:       08-04-11
		# Purpose:    Keep t_rf_ransparency for png and gif image
		# Param in:
		# Param out:  n/a
		# Reference:
		# Notes:
		#
	{
		// *** If PNG, perform some t_rf_ransparency retention actions (gif untested)
		if (in_array($this->fileExtension, $this->t_rf_ransparentArray) && $this->keepTransparency)
		{
			imagealphablending($im, false);
			imagesavealpha($im, true);
			$t_rf_ransparent = imagecolorallocatealpha($im, 255, 255, 255, 127);
			imagefilledrectangle($im, 0, 0, $width, $height, $t_rf_ransparent);
		}
		else
		{
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
		...
		#
	{
		...
		$t_rf_ransparent = imagecolorallocatealpha($filter, 255, 255, 255, 127);
		imagefilledrectangle($filter, 0, 0, $width, $height, $t_rf_ransparent);
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
	...
	public function addReflection($reflectionHeight = 50, $startingTransparency = 30, $inside = false, $bgColor = '#fff', $stretch = false, $divider = 0)
	{
		...
		$t_rf_ransaprencyAmount = $this->invertTransparency($startingTransparency, 100);
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
	...
	public function rotate($value = 90, $bgColor = 't_rf_ransparent')
		# Author:     Jarrod Oberto
		# Date:       07-05-2011
		...
		#
	{
		if ($this->imageResized)
		{
			...
			// *** Rotate
			$this->imageResized = imagerotate($this->imageResized, $degrees, $bg); // Rotate 45 degrees and allocated the t_rf_ransparent colour as the one to make t_rf_ransparent (obviously)

			// Ensure alpha t_rf_ransparency
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
	...
	public function roundCorners($radius = 5, $bgColor = 't_rf_ransparent')
		# Author:     Jarrod Oberto
		# Date:       19-05-2011
		...
		#
	{
		...
		// *** Check if the user wants t_rf_ransparency
		$isTransparent = false;
		if ( ! is_array($bgColor))
		{
			if (fix_strtolower($bgColor) == 't_rf_ransparent')
			{
				$isTransparent = true;
			}
		}
		...
		// *** If we use t_rf_ransparency, we need to color our curved mask with a unique color
		if ($isTransparent)
		{
		...
		// *** Replace the mask color (black) to t_rf_ransparent
		imagecolort_rf_ransparent($cornerImg, $maskColor);
		...
		// *** If corners are to be t_rf_ransparent, we fill our chromakey color as t_rf_ransparent.
		if ($isTransparent)
		{
			//imagecolort_rf_ransparent($this->imageResized, $imagebgColor);
			$this->imageResized = $this->t_rf_ransparentImage($this->imageResized);
			imagesavealpha($this->imageResized, true);
		}
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
	...
	public function addShadow($shadowAngle = 45, $blur = 15, $bgColor = 't_rf_ransparent')
		#
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
	...
	public function addCaptionBox($side = 'b', $thickness = 50, $padding = 0, $bgColor = '#000', $t_rf_ransaprencyAmount = 30)
		#
		# Author:   Jarrod Oberto
		# Date:   26 May 2011
		...
		#       (int) $t_rf_ransaprencyAmount: The amount of t_rf_ransparency to be
		#       applied.
		# Params out: n/a
		# Notes:
		#
	{
		...
		$t_rf_ransaprencyAmount = $this->invertTransparency($t_rf_ransaprencyAmount, 127, false);
		$t_rf_ransparent = imagecolorallocatealpha($this->imageResized, $r, $g, $b, $t_rf_ransaprencyAmount);
		imagefilledrectangle($this->imageResized, $positionArray['x1'], $positionArray['y1'], $positionArray['x2'], $positionArray['y2'], $t_rf_ransparent);
	}
...
## --------------------------------------------------------
		...
		#       third party class to save as bmp.
	{
		...
		if ( !is_resource($this->imageResized) && !is_object($this->imageResized))
		{
			if ($this->debug)
			{
...
## --------------------------------------------------------
	...
	public function setFillColor($value)
		...
		#
		# If the keepTransparency is set to false, then no t_rf_ransparency is to be used.
...
## --------------------------------------------------------
		...
		#
	{
		...
		{
			if (fix_strtolower($value) == 't_rf_ransparent')
			{
	...
	private function invertTransparency($value, $originalMax, $invert = true)
		...
		#       2) Inverts value to 100 is not t_rf_ransparent while 0 is fully
		#          t_rf_ransparent (like Photoshop)
	{
		// *** Test max range
	...
	private function t_rf_ransparentImage($src)
	{
		// *** making images with white bg t_rf_ransparent
		$r1 = 0;
		$g1 = 255;
		$b1 = 0;
		for ($x = 0; $x < imagesx($src); ++$x)
		{
			for ($y = 0; $y < imagesy($src); ++$y)
			{
				...
				for ($i = 0; $i < 270; $i++)
				{
					...
					{
						//if ($g == 255) {
						$t_rf_rans_colour = imagecolorallocatealpha($src, 0, 0, 0, 127);
						imagefill($src, $x, $y, $t_rf_rans_colour);
					}
				}
			}
		}
