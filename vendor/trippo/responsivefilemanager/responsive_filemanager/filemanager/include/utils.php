...
if ( ! function_exists('t_rf_rans'))
{
	...
	*
	* @return string t_rf_ranslated variable
	*/
	function t_rf_rans($var)
	{
		global $lang_vars;
...
* @param  string  $name             New file name without extension
* @param  bool    $t_rf_ransliteration
*
* @return bool
*/
...
function makeSize($size)
{
	...
	return (number_format($size, 0) . " " . t_rf_rans($units[ $u ]));
}
...
function fix_filename($str, $config, $is_folder = false)
{
	...
	if ($config['t_rf_ransliteration'])
	{
		if (!mb_detect_encoding($str, 'UTF-8', true))
		{
			$str = utf8_encode($str);
		}
		if (function_exists('t_rf_ransliterator_t_rf_ransliterate'))
		{
			$str = t_rf_ransliterator_t_rf_ransliterate('Any-Latin; Latin-ASCII', $str);
		}
		else
		{
	...
	// Empty or incorrectly t_rf_ransliterated filename.
