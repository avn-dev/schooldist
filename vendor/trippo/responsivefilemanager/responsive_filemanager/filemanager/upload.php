...
try {
    ...
    if ($_SESSION['RF']["verify"] != "RESPONSIVEfilemanager") {
        response(t_rf_rans('forbidden') . AddErrorLocation(), 403)->send();
        exit;
    }
    ...
    $messages = null;
    if (t_rf_rans("Upload_error_messages") !== "Upload_error_messages") {
        $messages = t_rf_rans("Upload_error_messages");
    }
    ...
    if (!checkresultingsize($_FILES['files']['size'][0])) {
        ...
        $upload_handler->response['files'][0]->error = sprintf(t_rf_rans('max_size_reached'), $config['MaxSizeTotal']) . AddErrorLocation();
